    ...
    public class MangaDexSource : IComicSource
    {
        ...
        )
        {
            ...
            var comics = searchResults
                ...
                {
                    var coverUrl = result
                        .Relationships.FirstOrDefault(r => r.Type == "cover_art")
                        ?.Attributes
                        is { } coverAttributes
        ...
        public async Task<Comic?> GetDetailsAsync(string comicId, CancellationToken ct = default)
        {
            ...
            MangaDexComic? detailsResult = (
                await GetFromApiAsync<DetailsResponse>(detailsUrl, ct)
            )?.Data;

            if (detailsResult is null)
                return null;

            var author = detailsResult
                .Relationships.FirstOrDefault(r => r.Type == "author")
                ?.Attributes
                is { } authorAttributes
                ? GetNameFromAttributes(authorAttributes)
                : null;

            var coverUrl = detailsResult
                .Relationships.FirstOrDefault(r => r.Type == "cover_art")
                ?.Attributes
                is { } coverAttributes
        ...
        public async Task<Chapter?> GetChapterDetailsAsync(
            string comicId,
            string chapterId,
            CancellationToken ct = default
        )
        ...
        {
            if (
                attributes is JsonElement element
